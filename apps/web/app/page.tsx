...
// Removed: import { Dialog, DialogContent, DialogTrigger } from "@/components/tailwind/ui/dialog"; // Import Dialog components
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-50">
      ...
      <header className="sticky top-0 z-10">
        ...
        <div className="flex h-14 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            {/* Typo Check Button - Modified onClick */}
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setIsTypoCheckOpen(true)} // Directly set state
            >
              <SpellCheck className="h-5 w-5 text-gray-600" />
            </Button>

            {/* Settings Button - Modified onClick */}
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setIsSettingsOpen(true)} // Directly set state
            >
              <Settings className="h-5 w-5 text-gray-600" />
            </Button>

            {/* Outline Button - Modified onClick */}
            <Button
              variant="ghost"
              size="icon"
              onClick={() => setIsOutlineOpen(true)} // Directly set state
            >
      ...
      <footer className="flex h-16 items-center justify-center border-t bg-white p-4 sticky bottom-0 z-10">
        {/* AI Toolbox Trigger Button - Modified onClick */}
        <Button
          ...
          onClick={() => setIsAiToolboxOpen(true)} // Directly set state
        >
          <span className="mr-2">✨</span> AI工具箱
        </Button>
      </footer>
      {/* Conditionally Rendered Panels */}
      {isAiToolboxOpen && <AiToolboxDialogContent onClose={() => setIsAiToolboxOpen(false)} />}
